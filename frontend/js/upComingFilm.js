...
document.getElementById('addSpiceForm').addEventListener('submit', function(event) {
  ...
  fetch('http://localhost:8080/api/v1/up-film/save', {
    method: 'POST',
    body: formData
  })
    .then(response => {
...
const getAllCustomer = () => {
  $.ajax({
    url: 'http://localhost:8080/api/v1/up-film/get',
    type: "GET",
    success: (res) => {
      console.log(res);
...
const deleteCustomer=(id) =>{
  $.ajax({
    url:`http://localhost:8080/api/v1/up-film/delete/${id}`,
    type:'DELETE',
    success:(res)=>{
      console.log(res)
      getAllCustomer()
...
$('#btn_update_customer').click((e) => {
  ...
  $.ajax({
    url: 'http://localhost:8080/api/v1/up-film/update', // Your API endpoint
    type: 'PUT',
